...
public class OptimalKUsageExample {
    ...
    public static void main(String[] args) {
        ...
        try {
            ...
            DirectedFeedbackVertexSetSolver<String, DefaultEdge> solver = new DirectedFeedbackVertexSetSolver<>(graph);

            // Try with computed optimal k
            boolean hasSolution = solver.solve(result.getOptimalK());
            System.out.println("DFVS solver with k=" + result.getOptimalK() + ": "
                    + (hasSolution ? "Solution found" : "No solution"));

            if (hasSolution) {
                System.out.println("DFVS solution: " + solver.getSolution());
            }
    ...
    private static void testGraph(
        ...
        boolean hasOptimalSolution = solver.solve(result.getOptimalK());
